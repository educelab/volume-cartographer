...
/************************************************************************************
 *                                                                                  *
 *  LRUCacheTest.cpp - tests the functionality of /v-c/common/datatypes/LRUCache.h  *
...
struct ReferenceBadKeyFixture{
    ...
    ReferenceBadKeyFixture(){
        ...
        //cache will push new pairs to front when adding, so final list order will be 99,98,...,0
...
BOOST_FIXTURE_TEST_CASE(CheckAbilityToResizeCache, ResizingCacheFixture) {
    ...
    BOOST_CHECK_EQUAL(_DefaultCache.capacity(), 200);             //check original capacity of 200
    BOOST_CHECK_EQUAL(_DefaultCache.size(), 0);                   //check that item list is empty
    _DefaultCache.setCapacity(_in_NewCap);                        //change capacity
    BOOST_CHECK_EQUAL(_DefaultCache.capacity(), 50);              //check new capacity
    BOOST_CHECK_EQUAL(_DefaultCache.size(), 0);                   //check that item list is still empty
}
...
BOOST_FIXTURE_TEST_CASE(CheckReferenceToOutOfBoundsKey, ReferenceBadKeyFixture) {
    ...
    for (size_t key = 0; key < _Cache.capacity(); key++){
        ...
        BOOST_CHECK_EQUAL(_Cache.get(key), key * key);            //check that values are correct
    }

    try{
        _Cache.get(_Cache.capacity() - ( _Cache.capacity() + 1 ));      //try to reference key < 0
...
BOOST_FIXTURE_TEST_CASE(InsertIntoCacheAndConfirmExistence, ReferenceBadKeyFixture){

    BOOST_CHECK_EQUAL(_Cache.capacity(), _Cache.size());                       //check capacity and size are equal
    BOOST_CHECK_EQUAL(_Cache.capacity(), 100);                                 //check cap is 100 (size implied)

    _Cache.put(_Cache.capacity(), (_Cache.capacity() * _Cache.capacity() ));   //insert item pair

    BOOST_CHECK(_Cache.exists(100));                                        //check that key == 100 exists now

    BOOST_CHECK_EQUAL(_Cache.capacity(), 100);                              //capacity and size should be unchanged
    BOOST_CHECK_EQUAL(_Cache.size(), 100);

    BOOST_CHECK_EQUAL(_Cache.exists(0), false);                             //check that item key == 0 was popped

    BOOST_CHECK_EQUAL(_Cache.get(100), 10000);                              //check that we can access newly added pair
...
BOOST_FIXTURE_TEST_CASE(CheckCacheWithDifferingCapacityAndSize, CreateCacheWithDefaultConstructor){
    ...
    for (size_t key = 0; key < _DefaultCache.capacity(); key += 2){      //fill item list halfway
    ...
    BOOST_CHECK_EQUAL(_DefaultCache.capacity(), 200);                    //now size should be 100 and capacity still 200
    ...
    BOOST_CHECK(_DefaultCache.exists(100));                      //check that keys 100 and 198 still exist
...
BOOST_FIXTURE_TEST_CASE(TryToInsertMorePairsThanCurrentCapacity, CreateCacheWithDefaultConstructor){
    ...
    for (size_t key = 0; key <= _DefaultCache.capacity(); key++){      //fill item list 1 past capacity

        _DefaultCache.put(key, key + key );
    }

    BOOST_CHECK_EQUAL(_DefaultCache.capacity(), 200);                    //cache should still have capacity of 200
    BOOST_CHECK_EQUAL(_DefaultCache.size(), 200);                          //size should be 200

    BOOST_CHECK_EQUAL(_DefaultCache.exists(0), false);                   //item key 0 should be popped with the last
...
BOOST_FIXTURE_TEST_CASE(PurgeTheCache, ReferenceBadKeyFixture) {
    ...
    BOOST_CHECK_EQUAL(_ Cache.exists(1), false);    //item key 1 should not exist (implied by 0 size)
