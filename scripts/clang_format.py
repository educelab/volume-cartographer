...
def callo(args):
    ...
    """
    return str(subprocess.check_output(args))
...
def get_tar_path(version, tar_path):
    ...
    return CLANG_FORMAT_SOURCE_TAR_BASE.substitute(
        version=version, tar_path=tar_path
    )
...
def get_clang_format_from_cache_and_extract(url, tarball_ext):
    ...
    # Download from file
    print(
        'Downloading clang-format %s from %s, saving to %s' %
...
def get_clang_format_from_darwin_cache(dest_file):
    ...
    get_clang_format_from_cache_and_extract(
        CLANG_FORMAT_HTTP_DARWIN_CACHE, '.xz'
    )

    # Destination Path
    shutil.move(
        get_tar_path(CLANG_FORMAT_VERSION, 'x86_64-apple-darwin'), dest_file
    )
...
def get_clang_format_from_linux_cache(dest_file):
    ...
    """
    get_clang_format_from_cache_and_extract(
        CLANG_FORMAT_HTTP_LINUX_CACHE, '.gz'
    )
    ...
    def __init__(self, path, cache_dir):
        ...
        if sys.platform == 'win32':
            ...
            for programfile in programfiles:
                win32bin = os.path.join(
                    programfile, 'LLVM\\bin\\clang-format.exe'
                )
...
# Assumes this script lives in //scripts/clang_format.py, so get two levels
# above that
def get_base_dir():
...
class Repo(object):
    ...
    def _callgito(self, args):
        ...
        return callo(['git', '-C', self.path] + args)
    ...
    def _callgit(self, args):
        ...
        # but we use these to support versions pre 1.8.5 but it depends on the command
        # and what the current directory is
        return subprocess.call(['git', '-C', self.path] + args)
    ...
    def _git_ls_files(self, cmd):
        ...
        file_list = [
            line.strip() for line in gito.splitlines()
            if not (line.startswith('vc-deps') or line.startswith('build'))
        ]

        files_match = re.compile('\\.(h|c|cpp|hpp)$')
        return [a for a in file_list if files_match.search(a)]
    ...
    def get_working_tree_candidate_files(self):
        """
        Query git to get a list of all files in the working tree to consider for
        analysis
        """
    ...
    def get_working_tree_candidates(self):
        ...
        Returns the full path to the file for clang-format to consume.
        """
        valid_files = self.get_working_tree_candidate_files()
...
def get_files_to_check_working_tree():
    ...
    return list(
        itertools.chain.
        from_iterable([r.get_working_tree_candidates() for r in get_repos()])
    )


def get_files_to_check():
    """
    Get a list of files that need to be checked based on which files are managed
    by git.
    """
    return list(
        itertools.chain.
        from_iterable([r.get_candidates(None) for r in get_repos()])
    )


def get_files_to_check_from_patch(patches):
    """
    Take a patch file generated by git diff, and scan the patch for a list of
    files to check.
    """
    ...
    valid_files = list(
        itertools.chain.
        from_iterable([r.get_candidates(candidates) for r in get_repos()])
    )
...
def _get_build_dir():
    """
    Get the location of the scons' build directory in case we need to download
    clang-format
    """
...
def _lint_files(clang_format, files):
    ...
    lint_clean = parallel_process(
        [os.path.abspath(f) for f in files], clang_format.lint
    )
