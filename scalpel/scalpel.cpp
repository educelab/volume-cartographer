...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < volpkg.getNumberOfSlices(); ++i) {
        volume.push_back(volpkg.volume().getSliceData(i));
        ...
        if (field.size() == 3) {
            ...
            outfile << std::setw(num_characters) << std::setfill('0') << index
                    << ".pcd";
            std::cout << "\rSaving tensor " << index - 1 << "/" << totalTensors
                      << std::flush;
            pcl::io::savePCDFileBinaryCompressed(
                path + "surface_normals/" + outfile.str(),
                process_tensors(field, index));
            field.pop_front();
        }
    }
...
Cloud process_tensors(TensorDeque field, int index)
{
    ...
    for (int i = 1; i < rows - 1; ++i) {
        for (int j = 1; j < cols - 1; ++j) {
            average[i][j] = field[0][i][j] + field[1][i][j] + field[2][i][j] +
                            field[1][i + 1][j] + field[1][i - 1][j] +
                            field[1][i][j + 1] + field[1][i][j - 1];
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            ...
            cv::Matx<double, 1, DIMENSION> normal_vector =
                eigenvectors.row(eigen_data.first);
