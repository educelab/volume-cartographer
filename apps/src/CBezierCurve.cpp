...
CBezierCurve::CBezierCurve( int nSampleInterval ) :
    fNumControlPoints( 4 ),
    fSampleInterval( nSampleInterval )
{
...
// Constructor
CBezierCurve::CBezierCurve( const std::vector< Vec2< double > > &nControlPoints,
                            int nSampleInterval ) :
    fNumControlPoints( 4 ),
    fSampleInterval( nSampleInterval )
{
...
// Set control points
void CBezierCurve::SetControlPoints( const std::vector< Vec2< double > > &nControlPoints )
{
    assert( nControlPoints.size() == fNumControlPoints ); // 4
...
// Draw curve on image
void CBezierCurve::DrawOnImage( cv::Mat &nImg,
                                const cv::Scalar &nColor )
{
//    circle( nImg, cv::Point2f( 1000, 500 ), 55, cv::Scalar( 0, 0, 255 ) );
    // REVISIT - FILL ME HERE
#ifdef _DEBUG
    cv::line( nImg, cv::Point2f( fControlPoints[ 0 ][ 0 ], fControlPoints[ 0 ][ 1 ] ), cv::Point2f( fControlPoints[ 1 ][ 0 ], fControlPoints[ 1 ][ 1 ] ), cv::Scalar( 0, 0, 255 ), 3 );
    cv::line( nImg, cv::Point2f( fControlPoints[ 1 ][ 0 ], fControlPoints[ 1 ][ 1 ] ), cv::Point2f( fControlPoints[ 2 ][ 0 ], fControlPoints[ 2 ][ 1 ] ), cv::Scalar( 0, 255, 0 ), 2 );
    cv::line( nImg, cv::Point2f( fControlPoints[ 2 ][ 0 ], fControlPoints[ 2 ][ 1 ] ), cv::Point2f( fControlPoints[ 3 ][ 0 ], fControlPoints[ 3 ][ 1 ] ), cv::Scalar( 255, 0, 0 ), 1 );
#endif // _DEBUG

    float aTotalLen = pythag( fControlPoints[ 0 ][ 0 ] - fControlPoints[ 1 ][ 0 ], 
                              fControlPoints[ 0 ][ 1 ] - fControlPoints[ 1 ][ 1 ] ) +
                      pythag( fControlPoints[ 1 ][ 0 ] - fControlPoints[ 2 ][ 0 ], 
                              fControlPoints[ 1 ][ 1 ] - fControlPoints[ 2 ][ 1 ] ) +
    ...
    for ( int i = 0; i < aNumOfPts; i++ ) {
        ...
        if ( i == 0 ) {
            ...
            cv::line( nImg, cv::Point2f( prev_x, prev_y ), cv::Point2f( xxx, yyy ), nColor/*cv::Scalar( 0, 0, 255 )*/ );
//            circle( nImg, cv::Point2f( xxx, yyy ), 5, cv::Scalar( 0, 0, 255 ) );
            prev_x = xxx;
            prev_y = yyy;
        }
...
#ifdef _DEBUG
        circle( nImg, cv::Point2f( fControlPoints[ 0 ][ 0 ], fControlPoints[ 0 ][ 1 ] ), 7, cv::Scalar( 255, 0, 0 ) );
        circle( nImg, cv::Point2f( fControlPoints[ 1 ][ 0 ], fControlPoints[ 1 ][ 1 ] ), 7, cv::Scalar( 255, 0, 0 ) );
        circle( nImg, cv::Point2f( fControlPoints[ 2 ][ 0 ], fControlPoints[ 2 ][ 1 ] ), 7, cv::Scalar( 255, 0, 0 ) );
        circle( nImg, cv::Point2f( fControlPoints[ 3 ][ 0 ], fControlPoints[ 3 ][ 1 ] ), 7, cv::Scalar( 255, 0, 0 ) );
#endif // _DEBUG
    }
}
