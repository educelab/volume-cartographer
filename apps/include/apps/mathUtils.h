...
#define SIGN(a,b) ((b) >= 0.0 ? fabs(a) : -fabs(a)) // from nrutil.h, www.nr.com/pubdom/nrutil.h.txt
#endif // SIGN
#ifndef SQR
#define SQR(x) ((x)*(x))
#endif // SQR
...
public:
	...
	Vec3( const CVectorN< int > &nSrc ) { val[ 0 ] = nSrc.Get( 0 ); val[ 1 ] = nSrc.Get( 1 ); val[ 2 ] = nSrc.Get( 2 ); }
    ...
    Vec3< T > operator+( const Vec3< T > &nRHS ) const { return Vec3< T >( val[ 0 ] + nRHS[ 0 ], val[ 1 ] + nRHS[ 1 ], val[ 2 ] + nRHS[ 2 ] ); }
...
public:
	...
	Vec4( T val0, T val1, T val2, T val3 ) { val[ 0 ] = val0; val[ 1 ] = val1; val[ 2 ] = val2; val[ 3 ] = val3; }
