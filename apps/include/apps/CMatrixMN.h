...
public:
    ...
    CMatrixMN& operator=( const CMatrixMN< T > &nCpy ); // REVISIT - remember, return reference, not a copy!
    ...
    {
        // REVISIT - IMPROVE - this is far far far less efficient; use vector/batch operation
    ...
    void Fill( int nLTRow, int nLTCol, int nBRRow, int nBRCol, const CMatrixMN &nSubMatrix )
    {
        // REVISIT - IMPROVE - this is far far far less efficient; use vector/batch operation
        //           for example, std::copy
        for ( int i = nLTRow; i < nBRRow; ++i ) {
            for ( int j = nLTCol; j < nBRCol; ++j ) {
                *( fData + i * fCols + j ) = nSubMatrix.Get( i - nLTRow, j - nLTCol );
            }
        }
    }
    // Get sub matrix
    ...
    double CalcNormFrobenius( void )
    {
        ...
        for ( int i = 0; i < fRows; ++i ) {
            for ( int j = 0; j < fCols; ++j ) {
                aResult += ( *( fData + i * fCols + j ) * *( fData + i * fCols + j ) );
            }
        }
        return sqrt( aResult );
    }
...
inline CMatrixMN< T >::CMatrixMN( int nRows, int nCols ) :
    ...
    fData( NULL )
{
    if ( nRows * nCols != 0 ) { // make sure fData is NULL when dimension is not set
...
inline CMatrixMN< T >::CMatrixMN( const CMatrixMN< T > &nCpy ) :
    ...
    fData( NULL )
{
    if ( fRows * fCols != 0 ) { // make sure fData is NULL when dimension is not set
        fData = new T[ fRows * fCols ];
