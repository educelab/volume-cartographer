...
public:
	...
	inline double operator()( double a, unsigned char b ) {
...
private:
	...
	using std::transform;
#else
	template< typename InputIterator, typename InputIterator2, typename OutputIterator, typename BinaryOperator >
	static OutputIterator transform( InputIterator first1,
									 InputIterator last1,
					...
					int niters )
{
	...
	for ( i = 0; i < niters; ++i ) {
		...
		for ( count = 0; count < ( int )Rs.size(); ++count ) {
			transform< MatIterator_< double >, 
						MatConstIterator_< unsigned char >, 
						MatIterator_< double >, 
						AddFloatToCharScaled > 
				( Rs[ count ].begin(),
				  Rs[ count ].end(),
				  observations[ count ].begin< unsigned char >(),
				  Rs[ count ].begin(),
