...
namespace volcart {
  namespace meshing {
    ...
    itk2bullet::itk2bullet( VC_MeshType::Pointer input, btSoftBodyWorldInfo& worldInfo, btSoftBody** output ) {

        VC_PointType old_point;
        btVector3* new_point;

        for ( VC_PointsInMeshIterator it = input->GetPoints()->Begin(); it != input->GetPoints()->End(); ++it ) {
