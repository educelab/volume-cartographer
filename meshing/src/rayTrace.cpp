...
namespace volcart {
  namespace meshing {
    ...
    std::vector<cv::Vec6f> rayTrace(ITKMesh::Pointer itkMesh, int aTraceDir, int width, int height, std::map<int, cv::Vec2d> uvMap) {
      ...
      for (int z = (int)bounds[4]; z < (int)bounds[5]; ++z) {
        ...
        for (double r = 0; r < PI_X2; r += D_THETA, ycount++) {
          ...
          if ( intersectPoints->GetNumberOfPoints() > 0 ) {
            ...
            point[3] = vtkMesh->GetCellData()->GetNormals()->GetTuple(intersectCells->GetId(0))[0];
            point[4] = vtkMesh->GetCellData()->GetNormals()->GetTuple(intersectCells->GetId(0))[1];
            point[5] = vtkMesh->GetCellData()->GetNormals()->GetTuple(intersectCells->GetId(0))[2];
