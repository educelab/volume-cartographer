...
std::vector<std::vector<Voxel>> OpticalFlowSegmentationClass::interpolatePoints(std::vector<std::vector<Voxel>> points, int window_size, bool backwards) {
    ...
    //     return points;
    // }
    // std::cout << "starting actual interpolation..." << std::endl;
    // Interpolate points from 0 to window_size * masterCloud_.width(), under backwards flag
...
OpticalFlowSegmentationClass::PointSet OpticalFlowSegmentationClass::compute()
{
    ...
    for (int zIndex = startIndex; !backwards ? zIndex > backwards_endIndex : zIndex < backwards_endIndex;
    ...
    // Duplicate the starting chain
    currentVs = startingChain_;
    FittedCurve evenlyStartingCurve(currentVs,  startIndex);
    currentVs = evenlyStartingCurve.evenlySpacePoints();
    // Add starting line
    ...
    //     std::cout << points[i][0][2] << " " << points[i][points[i].size()-1][2] << std::endl;
    // }
    /////////////////////////////////////////////////////////
    // Update progress
    progressComplete();
