                                       ...
                                       const int32_t numRandomTries)
{
    ...
    for (int32_t sliceIndex = startIndex_; sliceIndex <= endIndex_;
        ...
        // First chain object
        std::unique_ptr<Chain> currentChain(new Chain(pkg_, currentPos, sliceIndex));
        mesh.addChain(*currentChain);

        // Get current derivatives
        int32_t N = currentChain->size();
        std::vector<double> currentDerivs(N);
        currentDerivs.reserve(N);
        for (int32_t i = 0; i < N; ++i) {
            currentDerivs[i] = fivePointStencil(i, currentChain->positions());
        }
        ...
        // And evaluate it
        std::vector<double> bestIndepPosDeriv;
        bestIndepPosDeriv.reserve(N);
        for (int32_t i = 0; i < N; ++i) {
            bestIndepPosDeriv.push_back(fivePointStencil(i, bestPositions));
        }
        double minDerivativeDiff =
            l2_difference_norm(bestIndepPosDeriv, currentDerivs);
        ...
        for (int32_t i = 0; i < numRandomTries; ++i) {
            ...
            if (norm - minDerivativeDiff < derivativeTolerance) {
                ...
                bestPositions = std::move(positions);
            }
        }
