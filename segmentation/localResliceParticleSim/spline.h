...
public:
        ...
        : curveLength_(curveLength(xs, ys))
    {
        ...
        spline_ =
            Eigen::SplineFitting<SplineType>::Interpolate(points_, Degree);
...
private:
    ...
    // Calculate the knots for the curve (the points at which the curve will
    // interpolate
    // exactly) on the curve
    Eigen::VectorXd calculateKnots(const ScalarVector& xs,
                                   const ScalarVector& ys) const
    {
