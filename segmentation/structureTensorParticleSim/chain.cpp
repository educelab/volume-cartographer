...
Chain::Chain(pcl::PointCloud<pcl::PointXYZRGB>::Ptr segPath, VolumePkg* volpkg,
             double gravity_scale, int threshold, int endOffset,
             double spring_constant_k)
{
    ...
    for (pcl::PointCloud<pcl::PointXYZRGB>::iterator path_it = segPath->begin();
         path_it != segPath->end(); ++path_it) {
    ...
    _target_index =
        ((endOffset == DEFAULT_OFFSET)
             ? (volpkg->getNumberOfSlices() - 3)  // Account for zero-indexing
                                                  // and slices lost in
...
pcl::PointCloud<pcl::PointXYZRGB> Chain::orderedPCD()
{
    ...
    for (std::list<std::vector<Particle>>::iterator it = _history.begin();
        ...
        for (int i = 0; i < _chain_length; ++i) {
            ...
            pcl::PointXYZRGB point;
            point.x =
                row_at[i](1);  // point.x == vol[x][ ][ ] == field[ ][x][ ]
            point.y =
                row_at[i](2);  // point.y == vol[ ][y][ ] == field[ ][ ][y]
            point.z =
                row_at[i](0);  // point.z == vol[ ][ ][z] == field[z][ ][ ]
