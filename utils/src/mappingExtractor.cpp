...
volcart::PointType lookup2Dto3D(int x, int y);
...
int main(int argc, char* argv[])
{
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            if (maskROI.at<unsigned char>(y, x) == 255) {
                ...
                volcart::PointType pt;
                volcart::PixelType normal;
                pt[0] = mapInfo(0);
                pt[1] = mapInfo(1);
                pt[2] = mapInfo(2);
    ...
    // generate the faces
    volcart::CellType::CellAutoPointer cellpointer;
    ...
    for (int i = 1; i < height; ++i) {
        for (int j = 1; j < width; ++j) {
            ...
            cellpointer.TakeOwnership(new volcart::TriangleType);
...
volcart::PointType lookup2Dto3D(int y, int x)
{
    volcart::PointType pt;
