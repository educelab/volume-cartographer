...
namespace volcart {
    namespace io {
        ...
        plyWriter::plyWriter( std::string outputPath, VC_MeshType::Pointer mesh, volcart::Texture texture ) {
            _outputPath = outputPath;
            _mesh = mesh;
            _texture = texture;
        };
        ...
        bool plyWriter::validate() {
            ...
            bool hasExt = ( _outputPath.extension() == ".PLY" || _outputPath.extension() == ".ply" );
        ...
        int plyWriter::write() {
            ...
            VC_Origin starting_origin = _texture.uvMap().origin(); // Capture the starting origin
            _texture.uvMap().origin(VC_ORIGIN_TOP_LEFT); // Ensure uvMap origin is relative to what we need it to be
        ...
        int plyWriter::_writeHeader() {
            ...
            if( _mesh->GetNumberOfCells() != 0 ) {
                _outputMesh << "element face " << _mesh->GetNumberOfCells() << std::endl;
        ...
        int plyWriter::_writeVertices() {
            ...
            for ( auto point = _mesh->GetPoints()->Begin(); point != _mesh->GetPoints()->End(); ++point ) {
                ...
                _outputMesh << point.Value()[0] << " " << point.Value()[1] << " " << point.Value()[2] << " ";
                _outputMesh << normal[0] << " " << normal[1] << " " << normal[2];

                // If the texture has images and a uv map, write texture info
                if ( _texture.hasImages() && _texture.hasMap() ) {

                    // Get the intensity for this point from the texture. If it doesn't exist, set to 0.
                    double intensity = _texture.intensity(point.Index());
                    if ( intensity != VC_TEXTURE_NO_VALUE )
                        intensity = cvRound(intensity * 255.0/65535.0); // map 16bit to 8bit
                    else
                        intensity = 0;

                    _outputMesh << " " << (int) intensity << " " << (int) intensity << " " << (int) intensity;
                }
