...
namespace io {
  ...
  public:
    ...
    objWriter( std::string outputPath, VC_MeshType::Pointer mesh, VC_UVMap uvMap, cv::Mat uvImg);

    void setPath( std::string path ) { _outputPath = path; };
    void setMesh( VC_MeshType::Pointer mesh ) { _mesh = mesh; };
    void setUVMap( VC_UVMap uvMap ) { _textCoords = uvMap; };
  ...
  private:
    ...
    VC_UVMap _textCoords; // UV map for points accessed by point index
