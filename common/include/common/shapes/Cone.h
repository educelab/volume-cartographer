        ...
        public:
            Cone( int radius = 2, int height = 5, int recursion_level = 5 ) {
                ...
                while( r != recursion_level ){
                    ...
                    circle_list.clear();
                    double theta_inc = ( 2*M_PI ) / ( ( r+2 ) * 8 );  // number of section to calculate angle at each vertex
                    double theta = 0.0 ;
                    for ( int p_id = 0 ; p_id < ( (r+2) * 8) ; p_id ++ ) {

                        c_point[0] = radius*cos(theta);          // x value that will be on circle
                        c_point[1] = radius*sin(theta);          // y value that will be on circle
                        c_point[2] = height;
                        circle_list.push_back(c_point);
                ...
                for ( auto point = circle_list.begin(); point != circle_list.end(); ++point ) {
