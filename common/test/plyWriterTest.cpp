    ...
    CompareFixtureMeshAndSavedMeshData, CreateITKPlaneMeshFixture)
{
    ...
    int p_id = 0;
    for (volcart::PointsInMeshIterator point =
             _in_PlaneMesh->GetPoints()->Begin();
         point != _in_PlaneMesh->GetPoints()->End(); ++point) {

        volcart::PixelType _in_PlaneMeshNormal;
    ...
    volcart::CellIterator _in_PlaneMeshCell =
        _in_PlaneMesh->GetCells()->Begin();
    ...
    while (_in_PlaneMeshCell != _in_PlaneMesh->GetCells()->End()) {
        ...
        volcart::PointsInCellIterator _in_PlaneMeshPoint =
