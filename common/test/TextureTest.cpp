...
struct EmptyConstructorTextureFixture{
    ...
    EmptyConstructorTextureFixture(){
        ...
        std::cerr << "Creating texture object using empty constructor..." << std::endl;

        //store metadata --> not needed
        _ID = _Texture.metadata().getString("id");
        _Type = _Texture.metadata().getString("type");
    ...
    ~EmptyConstructorTextureFixture(){std::cerr << "Destroying texture object..." << std::endl;}
...
struct TextureWithImageFixture{
    ...
    ~TextureWithImageFixture(){std::cerr << "Destroying texture object..." << std::endl;}
...
BOOST_FIXTURE_TEST_CASE(TestTextureEmptyConstructor, EmptyConstructorTextureFixture){
...
BOOST_FIXTURE_TEST_CASE(AddImageToTextureTest, TextureWithImageFixture) {
    ...
    for (int img_id = 0; img_id < _Texture.numberOfImages(); img_id++) {
        ...
        BOOST_CHECK_EQUAL_COLLECTIONS(_Texture.getImage(img_id).datastart, _Texture.getImage(img_id).dataend,
...
BOOST_FIXTURE_TEST_CASE(IntensityRetrievalFromTextureImageTest, TextureWithUVMapToCheckIntensityFixture) {
