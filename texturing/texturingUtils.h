                            ...
                            double                       nSampleInterval = 0.2 )
{
    ...
    double aCurValue = VolPkg.volume().getIntensityAtCoord( nPoint );

    for ( int i = 0; i < 6; ++i ) {
        if ( aCurValue < VolPkg.volume().getIntensityAtCoord( nPoint + aVecs[ i ] )) {
            aIsLocalMax = false;
            break;
        }
    }
                                   ...
                                   double                       *nData )        // [out] samples
{
    ...
    for ( int i = 0; aDataCnt < nSections; ++i ) {
        ...
        if ( nSamplingDir == 0 ) {
          ...
          double tmp = VolPkg.volume().getIntensityAtCoord( aPos );
...
inline double FilterIntersection( const cv::Vec3f              &nPoint,
                                  VolumePkg                    &VolPkg )
{
    return VolPkg.volume().getIntensityAtCoord( nPoint );
}
