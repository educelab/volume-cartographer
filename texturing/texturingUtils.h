...
inline bool IsLocalMaximum(const cv::Vec3f &nPoint, VolumePkg &VolPkg,
                       ...
                       double *nData)     // [out] samples
{
    ...
    cv::normalize(aNormalVec, aNormalVec, 0, range / 2, cv::NORM_MINMAX,
                  CV_32F);
            ...
            for (int k = 1; k < aSizeMinor; ++k) {
                ...
                if (j * nSampleInterval <
                    ...
                    if (nSamplingDir == 0) {  // sample along both positive and
                        ...
                        for (int t = 0; t < 8; ++t) {
                            aDir = i * nSampleInterval * aSign[t][0] *
                                   ...
                                   k * nSampleInterval * aSign[t][2] *
                                       aMinorAxisDir2;
                            // REVISIT - note that the points along the axis are
                            // counted multiple times
                            //           fixed this by starting from 1 instead
                            //           of 0, and add the points on axis first
                            aPos[0] = nCenter[0] + aDir[0];
                            aPos[1] = nCenter[1] + aDir[1];
                            aPos[2] = nCenter[2] + aDir[2];
                            ...
                            nData[aDataCnt] = tmp;  // REVISIT - we assume we
                                                    // have enough space
                            aDataCnt++;
                        }  // for t
                    } else if (nSamplingDir ==
                               1) {  // sample along positive direction
                        for (int t = 0; t < 4; ++t) {
                            ...
                            aDataCnt++;
                               ...
                               2) {  // sample along negative direction
                        for (int t = 0; t < 4; ++t) {
                            aDir = i * nSampleInterval *
                        ...
                        printf(
                            "WARNING: invalid parameter in "
                            "SamplingWithinEllipse: nSamplingDir %d\n",
                            nSamplingDir);
                        return;
                    }
                }  // if
    ...
    VC_Direction_Option nSamplingDir = DirectionOptionBoth)  // sample direction
{
    ...
    SamplingAlongNormal(nR1, nSampleDist, nPoint, nNormal, VolPkg, nSamplingDir,
                        aSamples, &aNumSamples, true);
    ...
    VC_Direction_Option nSamplingDir = DirectionOptionBoth)  // sample direction
{
    switch (nFilter) {
        ...
        case VC_Composite_Option::CompositeOptionMean:
            return FilterMean(nPoint, nNormal, VolPkg, nR1, nR2, nSampleDist,
                              nSamplingDir);
            break;
        case VC_Composite_Option::CompositeOptionNonMaximumSuppression:
            return FilterNonMaximumSuppression(nPoint, nNormal, VolPkg, nR1,
                                               nR2, nSampleDist, nSamplingDir);
            break;
        case VC_Composite_Option::CompositeOptionMax:
            return FilterMax(nPoint, nNormal, VolPkg, nR1, nR2, nSampleDist,
                             nSamplingDir);
            break;
        case VC_Composite_Option::CompositeOptionMin:
            return FilterMin(nPoint, nNormal, VolPkg, nR1, nR2, nSampleDist,
                             nSamplingDir);
            break;
        ...
        case VC_Composite_Option::CompositeOptionMedianAverage:
            return FilterMedianAverage(nPoint, nNormal, VolPkg, nR1, nR2,
                                       nSampleDist, nSamplingDir);
            break;
        default: return FilterIntersection(nPoint, VolPkg); break;
    }  // switch nFilter
}
