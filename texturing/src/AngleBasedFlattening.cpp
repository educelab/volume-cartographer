...
AngleBasedFlattening::AngleBasedFlattening(MeshType::Pointer mesh)
    : _mesh(mesh)
...
///// Get Output /////
// Get output as mesh
volcart::MeshType::Pointer AngleBasedFlattening::getMesh()
{
    volcart::MeshType::Pointer output = volcart::MeshType::New();
...
///// Parameters /////
void AngleBasedFlattening::setUseABF(bool a) { _useABF = a; };
void AngleBasedFlattening::setABFMaxIterations(int i)
{
    _maxABFIterations = i;
};
...
void AngleBasedFlattening::_fillHalfEdgeMesh()
{
    _heMesh.clear();

    ///// Vertices /////
    for (PointsInMeshIterator point = _mesh->GetPoints()->Begin();
...
void AngleBasedFlattening::_solve_abf()
{
    _computeSines();
    ...
    std::cerr << "volcart::texturing::abf: Iterations: " << i
              << " || Final norm: " << norm << " || Limit: " << _limit
              << std::endl;
}

///// Helpers - ABF /////
// Compute the sine and cosine for each angle in the HEM
void AngleBasedFlattening::_computeSines()
{
...
double AngleBasedFlattening::_computeGradient()
{
    double norm = 0.0;
...
double AngleBasedFlattening::_computeSinProduct(
    volcart::HalfEdgeMesh::VertPtr v)
{
...
double AngleBasedFlattening::_computeSinProduct(
    volcart::HalfEdgeMesh::VertPtr v, volcart::HalfEdgeMesh::IDType a_id)
{
    volcart::HalfEdgeMesh::EdgePtr e0, e1, e2;
    double sin1, sin2;
    sin1 = sin2 = 1.0;
...
bool AngleBasedFlattening::_invertMatrix()
{
    // Create a new solver + context
    bool success;
    LinearSolver* context;
    volcart::HalfEdgeMesh::IDType ninterior = _interior.size();

    context = EIG_linear_solver_new(0, ninterior * 2, 1);

    // Add the _bInterior points to RHS
    for (volcart::HalfEdgeMesh::IDType i = 0; i < _bInterior.size(); ++i)
    ...
    for (auto f = _heMesh.getFace(0); f; f = f->nextlink, ++counter) {
        ...
        if (v0->interior()) {
            ...
            _J2dt.at<double>(e1->id, 0) = j2[1][0] =
                _computeSinProduct(e0->vert, e1->id) * wi2;
            _J2dt.at<double>(e2->id, 0) = j2[2][0] =
                _computeSinProduct(e0->vert, e2->id) * wi3;

            EIG_linear_solver_right_hand_side_add(
                context, 0, i_id, j2[0][0] * beta[0]);
            EIG_linear_solver_right_hand_side_add(
                context, 0, ninterior + i_id,
        ...
        if (v1->interior()) {
            ...
            _J2dt.at<double>(e0->id, 1) = j2[0][1] =
                _computeSinProduct(e1->vert, e0->id) * wi1;
            _J2dt.at<double>(e1->id, 1) = j2[1][1] = 1.0f * wi2;
            _J2dt.at<double>(e2->id, 1) = j2[2][1] =
                _computeSinProduct(e1->vert, e2->id) * wi3;

            EIG_linear_solver_right_hand_side_add(
                context, 0, i_id, j2[1][1] * beta[1]);
            EIG_linear_solver_right_hand_side_add(
                context, 0, ninterior + i_id,
        ...
        if (v2->interior()) {
            ...
            _J2dt.at<double>(e0->id, 2) = j2[0][2] =
                _computeSinProduct(e2->vert, e0->id) * wi1;
            _J2dt.at<double>(e1->id, 2) = j2[1][2] =
                _computeSinProduct(e2->vert, e1->id) * wi2;
            _J2dt.at<double>(e2->id, 2) = j2[2][2] = 1.0f * wi3;

            EIG_linear_solver_right_hand_side_add(
                context, 0, i_id, j2[2][2] * beta[2]);
            EIG_linear_solver_right_hand_side_add(
                context, 0, ninterior + i_id,
    ...
    if (success) {
        for (auto f = _heMesh.getFace(0); f; f = f->nextlink) {
            ...
            if (v0->interior()) {
                ...
                double x2 = EIG_linear_solver_variable_get(
                    context, 0, ninterior + i_id);
        ...
        volcart::HalfEdgeMesh::IDType p_id, i_id;
        for (auto it = _interior.begin(); it != _interior.end(); ++it) {
            p_id = it->first;
            i_id = it->second;
            _heMesh.getVert(p_id)->lambdaPlanar +=
                EIG_linear_solver_variable_get(context, 0, i_id);
            _heMesh.getVert(p_id)->lambdaLength +=
                EIG_linear_solver_variable_get(context, 0, ninterior + i_id);
        }
    }
...
void AngleBasedFlattening::_solve_lscm()
{
    ...
    context = EIG_linear_least_squares_solver_new(
        2 * _heMesh.getNumberOfFaces(), 2 * _heMesh.getNumberOfVerts(), 1);
    ...
    EIG_linear_solver_variable_set(
        context, 0, 2 * _pin0, _heMesh.getVert(_pin0)->uv[0]);
    EIG_linear_solver_variable_set(
        context, 0, 2 * _pin0 + 1, _heMesh.getVert(_pin0)->uv[1]);
    EIG_linear_solver_variable_set(
        context, 0, 2 * _pin1, _heMesh.getVert(_pin1)->uv[0]);
    EIG_linear_solver_variable_set(
        context, 0, 2 * _pin1 + 1, _heMesh.getVert(_pin1)->uv[1]);
...
AngleBasedFlattening::_getMinMaxPointIDs()
{
