...
{
    ClothModelingUVMapping* w =
        static_cast<ClothModelingUVMapping*>(world->getWorldUserInfo());
    ...
    , _expansionPins(expansionPins)
{
    ...
    _WorldCollisionDispatcher =
        new btCollisionDispatcher(_WorldCollisionConfig);
    ...
    _World = new btSoftRigidDynamicsWorld(
        _WorldCollisionDispatcher, _WorldBroadphase, _WorldConstraintSolver,
        _WorldCollisionConfig, _WorldSoftBodySolver);
...
{
    if (_unfurlIterations > 0)
        _unfurl();
    if (_collideIterations > 0)
        _collide();
    if (_expandIterations > 0)
        _expand();
}
...
volcart::MeshType::Pointer ClothModelingUVMapping::getMesh()
{
...
void ClothModelingUVMapping::_unfurl()
{
    ...
    _softBody->m_materials[0]->m_kAST =
        1.0;  // Area/Angular stiffness coefficient [0,1]
    _softBody->m_materials[0]->m_kVST =
        1.0;  // Volume stiffness coefficient [0,1]
...
void ClothModelingUVMapping::_collide()
{
    ...
    for (uint16_t i = 0; i < _collideIterations; ++i) {
        std::cerr << "volcart::texturing::clothUV: Colliding " << i + 1 << "/"
                  << _collideIterations << "\r" << std::flush;
        _World->stepSimulation(1 / 60.f, 10);
        _softBody->solveConstraints();
    }
    std::cerr << std::endl;
...
double ClothModelingUVMapping::_SurfaceArea()
{
    ...
    for (int i = 0; i < _softBody->m_faces.size(); ++i) {
        ...
        a = _softBody->m_faces[i].m_n[0]->m_x.distance(
            _softBody->m_faces[i].m_n[1]->m_x);
        b = _softBody->m_faces[i].m_n[0]->m_x.distance(
            _softBody->m_faces[i].m_n[2]->m_x);
        c = _softBody->m_faces[i].m_n[1]->m_x.distance(
            _softBody->m_faces[i].m_n[2]->m_x);
