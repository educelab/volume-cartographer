...
int compositeTextureV2::_process()
{
    ...
    std::cerr << std::endl;
    std::cerr << "volcart::texturing::compositeTexturing:: Pixels not in cell: "
...
int compositeTextureV2::_generateCellInfo()
{
    ...
    ITKPoint centroid;
    std::cerr
        << "volcart::texturing::compositeTexturing: Generating cell information"
        << std::endl;
    for (auto cell = _input->GetCells()->Begin();
         cell != _input->GetCells()->End(); ++cell) {
...
cv::Vec3d compositeTextureV2::_BarycentricCoord(
    const cv::Vec3d& nXYZ,
    const cv::Vec3d& nA,
    const cv::Vec3d& nB,
    const cv::Vec3d& nC)
...
cv::Vec3d compositeTextureV2::_CartesianCoord(
    const cv::Vec3d& nUVW,
    const cv::Vec3d& nA,
    const cv::Vec3d& nB,
    const cv::Vec3d& nC)
