...
[USAGE] format_slices [-sp | --slices-path] <path to slices> [-of | --output-file] <name of output file>
OR
[USAGE] python format_scroll_data.py  [-sp | --slices-path] <path to slices> [-of | --output-file] <name of output file>

[NOTE] I recommend putting
alias format_slices='python <absolute path to this program>'
...
if "-of" in sys.argv:
	outpath = sys.argv[sys.argv.find("-f") + 1]
elif "--ouput-file" in sys.argv:
	outpath = sys.argv[sys.argv.find("--output-file") + 1] + '.volumepkg'
else:
	outpath = os.getcwd() + "/output.volumepkg"
